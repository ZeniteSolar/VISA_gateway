# Instruments
import serial
from meters.pa_1000 import Meter as PA1000, MeterMetrics as PA1000Metrics
...
try:
	for i in np.linspace(0, 70, 30):
		...
		while True:
			data = meter.measure_or_none()
			if data:
				# Calculates the ADC average
				adc_avg = np.average(adc_samples)
				# Writes in the file
				f_log_file.write(str(time.time()) + ',' + str(adc_avg) + ',' + str(data[0]) + '\n')
				f_log_file.flush()
				break
			else:
				data = serial_con.readline().decode("ascii")
